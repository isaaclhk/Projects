...
Like the bar plot, the summary plot above outputs the top 7 features sorted in descending order. However, this plot provides greater detail. It allows us to see the distribution of shapley and feature values for each feature, and reflects the relationships between feature values and shapley values. For example, based on the plot above, higher PCR medians generally contribute to greater risk of admission into ICU while higher linfocitos medians are protective against ICU admission.
